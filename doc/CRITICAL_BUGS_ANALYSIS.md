...
或者添加一个参数控制：
```javascript
function loadNextPage(page, total, tid, container, isBackground = false, metaData = null, preloadLimit = null) {
    // ...
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains('threads')) {
                    const threadStore = db.createObjectStore('threads', { keyPath: 'tid' });
                    threadStore.createIndex('lastAccess', 'lastAccess', { unique: false });
                }
                
                // 页面内容存储
                if (!db.objectStoreNames.contains('pages')) {
                    const pageStore = db.createObjectStore('pages', { keyPath: ['tid', 'page'] });
