...
class IndexedDBAdapter {
    ...
    async openDatabase() {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                console.log('[IndexedDB] 数据库升级，版本:', event.oldVersion, '->', event.newVersion);
    ...
    createObjectStores(db) {
        try {
            ...
            if (!db.objectStoreNames.contains('thread_meta')) {
                const threadMetaStore = db.createObjectStore('thread_meta', { keyPath: 'tid' });
                threadMetaStore.createIndex('lastAccess', 'lastAccess', { unique: false });
                threadMetaStore.createIndex('cacheTime', 'cacheTime', { unique: false });
                console.log('[IndexedDB] 创建 thread_meta 存储');
            }

            // 创建 page_content 存储
            if (!db.objectStoreNames.contains('page_content')) {
                const pageContentStore = db.createObjectStore('page_content', { keyPath: ['tid', 'page'] });
                pageContentStore.createIndex('tid', 'tid', { unique: false });
                pageContentStore.createIndex('timestamp', 'timestamp', { unique: false });
    ...
    async getByIndex(storeName, indexName, query) {
        try {
            ...
            console.error(`[IndexedDB] 按索引查询失败 (${storeName}.${indexName}):`, e);
            throw e;
        }
    }
