    ...
    function createProgressBar() {
        ...
        progressLine3.style.cssText = 'font-size: 11px; color: #d1d5db; display: none;';
    ...
    function initializeThreadView() {
        try {
            console.log('[初始化] tid:', tid, 'cacheManager:', !!cacheManager);
            ...
            console.log('[初始化] 元数据:', meta);
            console.log('[初始化] 配置:', config);
            
            if (meta) {
                const isValid = cacheManager.isCacheValid(config.cacheExpireTime);
                console.log('[初始化] 缓存有效性:', isValid);
                console.log('[初始化] lastAccess:', new Date(meta.lastAccess));
                console.log('[初始化] cacheExpireTime:', config.cacheExpireTime, '(毫秒)');
                ...
                if (isValid) {
                    ...
                    console.log('[NGA 楼中楼] 缓存已过期，清理并重新加载');
                    cacheManager.clearCache();
                }
            } else {
                console.log('[NGA 楼中楼] 无缓存元数据');
            }
            
            console.log('[NGA 楼中楼] 开始全新加载');
        ...
        try {
            ...
            setTimeout(() => {
                try {
                    ...
                    console.log(`[预加载] displayedPageCount=${displayedPageCount}, preloadPages=${config.preloadPages}, actualLimit=${actualLimit}, total=${total}`);
